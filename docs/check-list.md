...
* 분명 본인은 아무것도 안한 것 같은데, 비교적 최근부터 갑자기 봇이 작동하지 않는다면, [카카오톡 버전별 변동사항]({{site.url}}{{site.baseurl}}/docs/version-issue/)도 한 번 읽어보세요. 비교척 최근에 카카오톡 쪽에 변동이 있어서 봇이 정상적으로 작동하지 않을 수도 있어요.
...
### 1-5. 어느 앱의 알림을 읽을지 설정했나요?
* `봇 구동 앱`이 카카오톡이 아닌 다른 앱에 반응하도록 설정했는지 확인해주세요.
* 기본적으로, `채팅 자동응답 봇`과  `메신저봇`은 카카오톡에 반응하도록 설정되어있으니, 괜히 이것저것 건든 적이 없다면 별 문제는 없을거에요.
